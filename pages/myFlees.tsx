import {
  FETCH_FLEES_BY_RECEIVER,
  FETCH_FLEES_BY_SENDER,
  subgraphQuery,
} from "../components";
        ...
        {data &&
          data?.map((flee) => {
            const startTime = new Date(
              parseInt(flee.startTime) * 1000
            ).toLocaleString();
            const stopTime = new Date(
              parseInt(flee.stopTime) * 1000
            ).toLocaleString();
            const status =
              stopTime > new Date().toLocaleString() ? "Active" : "Completed";
            return (
              <div
              ...
              >
                <p className="w-96">
                  {!receiver ? flee.receiver : flee.sender}
                </p>
                <p className="w-10 ml-12">
                  {ethers.utils.formatEther(flee.amount)}
                </p>
                <p className="w-52 ml-20">{startTime}</p>
                <p className="w-52 ml-2">{stopTime}</p>
                <p className="ml-2 w-20">
                  {flee.cancelled ? "Canceled" : status}
                </p>
                {flee.cancelled === null && !receiver && status === "Active" ? (
                  <button
                    className="ml-16 p-2 rounded-xl bg-red-500"
                    onClick={() => cancelFlee(flee.id)}
                  >
                    Cancel
                  </button>
                ) : (
                ...
                {receiver && !flee.cancelled && status === "Completed" ? (
                  flee.claimed ? (
                    <p className="ml-16 p-2 rounded-xl bg-blue-500 px-4">
                      Claimed
                    </p>
                  ) : (
                    <button
                      className="ml-16 p-2 rounded-xl bg-green-600"
                      onClick={() => withdrawFlee(flee.id)}
                    >
                      Claim
                    </button>
                  )
