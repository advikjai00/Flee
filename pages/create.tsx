...
export default function Create() {
  ...
  function makeFileObjects(data: any) {
    ...
    const files = [new File([blob], address + ".json")];
    return files;
  }
  ...
  const create = async () => {
    try {
      ...
      const tokenAmount = ethers.utils.parseEther(
        (document.getElementById("tokenAmount") as HTMLInputElement).value
      );
      const receiverAddress = (
        document.getElementById("receiverAddress") as HTMLInputElement
      ).value;
      await giveAllowance();
      let data: any = {};
      ...
      const tx = await contract.createFlee(
        receiverAddress,
        tokenAddress,
        tokenAmount,
        startTimestamp,
    ...
    <div className=" text-light px-60 pt-10">
      ...
      <div className="flex gap-28">
        <div className="py-3 w-1/2">
          <h1 className="text-2xl font-['Work Sans']">Receiver address</h1>
          <input
            className="w-full mt-2 bg-gray-600 h-8 rounded-md focus:outline-none p-2"
            type="text"
        ...
        <div className="py-3 ">
          <h1 className="text-2xl font-['Roboto Condensed']">Start Time</h1>
          <input
            className=" mt-2 bg-gray-600 h-8 rounded-md focus:outline-none p-2"
            type="datetime-local"
            id="startTime"
          />
        </div>
      ...
      <div className="flex gap-28">
        <div className="py-3 w-1/2">
          <h1 className="text-2xl font-['Work Sans']">Token address</h1>
          <input
            className="w-full mt-2 bg-gray-600 h-8 rounded-md focus:outline-none p-2"
            type="text"
            placeholder="Enter address"
        ...
        <div className="py-3 ">
          <h1 className="text-2xl font-['Roboto Condensed']">End Time</h1>
          <input
            className=" mt-2 bg-gray-600 h-8 rounded-md focus:outline-none p-2"
            type="datetime-local"
            id="endTime"
      ...
      <div className="flex gap-28 ">
        <div className="py-3 w-1/2">
          <h1 className="text-2xl font-['Work Sans']">Amount</h1>
          <input
            className="w-xl mt-2 bg-gray-600 h-8 rounded-md focus:outline-none p-2"
            type="number"
        ...
        <div className="flex gap-4 py-14 items-center">
          <p className="text-2xl font-['Work Sans']">Available : </p>
          <p className="text-xl">{balance.toFixed(2)}</p>
          <button
            className="font-['Fasthand'] px-3 py-1 rounded-md bg-red-500"
            onClick={setMax}
          >
            Max
          </button>
      ...
      <div className="py-6 flex justify-center">
        <button
          className="bg-violet-800 text-2xl font-['Fasthand'] w-40 rounded-md p-2"
          onClick={create}
        >
          Create
        </button>
